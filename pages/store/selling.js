  ...
  const handleSubmit = async () => {
    try {
      ...
      if (files.length == 1) {
        ...
        let products_id = response?.data?.products_id;
        const filterIndexZero = files.filter((_, index) => index !== 0);
        for (let i = 0; i < filterIndexZero.length; i++) {
          const addPhotos = await axios.post(
            `${process.env.NEXT_PUBLIC_BACKEND_URL}/products/add/photo/${products_id}`,
            {
              product_picture: filterIndexZero[i],
            },
            {
              headers: {
                "Content-Type": "multipart/form-data",
        ...
        <div className="container">
          <div className="row">
            ...
            <div className="col-8">
              ...
              <div className="card text-start  mb-4">
                <div className="card-body">
                  <h5 className="card-title">PHOTO OF PRODUCTS</h5>
                  <hr />
